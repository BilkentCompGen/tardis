...
typedef struct strvar {
    ...
    char indNames[totalNumInd][strSize]; /* names of the individuals supporting the SV */
    bool low_qual;
...
struct strvar* new_strvar(char *chrName,int outer_start, int inner_start, int outer_end, int inner_end, enum SVTYPE svtype,
		int sr_sup, float avg_edit, int min_svlen, int max_svlen, char *samples, double conf_score, bool filtered, bool,
