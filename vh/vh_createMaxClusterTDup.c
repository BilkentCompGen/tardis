...
void vh_addToGenomeIndex_TDup (char *chroName)
{
	LibraryInfo *libInfo;
	...
	while (libInfo != NULL)
	{
		...
		while (divetReadMappingPtr != NULL)
		{
											...
											divetReadMappingPtr->locMapLeftEnd < maxDuplicationLen))
			{
				...
				//fprintf(stderr, "%s %d %d %d %d %c %c %lf\n", divetReadMappingPtr->chroName,
					//	divetReadMappingPtr->locMapLeftStart, divetReadMappingPtr->locMapLeftEnd, divetReadMappingPtr->locMapRightStart,
						//divetReadMappingPtr->locMapRightEnd, divetReadMappingPtr->orientationLeft, divetReadMappingPtr->orientationRight, divetReadMappingPtr->editDistance);
...
void vh_initializeReadMapping_TDup (char *chroName, int chroSize)
{
	int i;
	...
	vh_addToGenomeIndex_TDup (chroName);
