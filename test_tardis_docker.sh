...
# pull docker image. Comment this out if it is already pulled
docker pull alkanlab/tardis

# ready to call TARDIS 
docker run --user=$UID -w ${PWD} -v ${PWD}:/input -v ${PWD}:/output alkanlab/tardis  -i /input/$INPUTBAM --ref /input/$REF --sonic /input/$SONIC --out /output/NA11930-chrom20-demo
